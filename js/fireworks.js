(function ($, window) {
  $.fn.vfireworks = function (options) {
    var defaults = {
      launchTime: 5000,
      clickLaunch: false,
      showRoute: false,
    ...
    var Fireworks = function (vcontainer, opts) {
      var self = this;
      ...
      self.init = function (vcontainer) {
        self.dt = 0;
        self.oldTime = Date.now();
        self.canvas = document.createElement("canvas");
        self.canvasContainer = vcontainer;

        var canvasContainerDisabled = vcontainer.get(0);
        self.canvas.onselectstart = function () {
          return false;
        };

        var width = vcontainer.width();
        var height = vcontainer.height();

        self.canvas.width = self.cw = width;
        self.canvas.height = self.ch = height;

        self.particles = [];
        self.partCount = opts.partCount;
        self.fireworks = [];
        self.mx = self.cw / 2;
        self.my = self.ch / 2;
        self.currentHue = 170;
        self.partSpeed = opts.partSpeed;
        self.partSpeedVariance = opts.partSpeedVariance;
        self.partWind = 30;
        self.partFriction = opts.partFriction;
        self.partGravity = opts.partGravity;
        self.hueMin = opts.hueMin;
        self.hueMax = opts.hueMax;
        self.fworkSpeed = opts.fworkSpeed;
        self.fworkAccel = opts.fworkAccel;
        self.hueVariance = opts.hueVariance;
        self.flickerDensity = 20;
        self.showShockwave = true;
        self.showTarget = false;
        self.clearAlpha = opts.clearAlpha;
      ...
      self.init(vcontainer);

      var initialLaunchCount = Math.floor(opts.launchTime / self.density);
      while (initialLaunchCount--) {
        ...
        }, initialLaunchCount * self.density);
      }
    };

    var fworks = new Fireworks(vcontainer, opts);
    setTimeout(function () {
      vcontainer.fadeOut(5000, function () {
        vcontainer.empty();
        vcontainer.show();
